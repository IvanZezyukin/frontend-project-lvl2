    ...
    if (typeof value !== 'object' || value === null) {
      ...
      if (key in obj2) {
        if (value === obj2[key]) {
          ...
          result.push({ prefix: '  + ', key, value: String(obj2[key]) });
        }
      }
      ...
      if (key in obj2) {
        if (typeof obj2[key] === 'object') {
          result.push({ prefix: '    ', key, value: newDiff(value, obj2[key]) });
        } else {
          // что делаем если в первом объект, а во втором не объект
          result.push({ prefix: '  - ', key, value: newDiff(value, value) });
          result.push({ prefix: '  + ', key, value: obj2[key] });
        }
      }
      // если ключ удален
      if (!(key in obj2)) {
